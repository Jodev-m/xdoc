...
export function MobileNav() {
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around bg-white dark:bg-neutral-900 border-t dark:border-neutral-700 px-1 py-1 safe-area-bottom">
      <NavItem href="/" label="Accueil"><Home className="w-5 h-5" /></NavItem>
      <NavItem href="/search" label="Recherche"><Search className="w-5 h-5" /></NavItem>
      <NavItem href="/stats" label="Stats"><BarChart3 className="w-5 h-5" /></NavItem>
      <NavItem href="/versions" label="Historique"><Clock className="w-5 h-5" /></NavItem>
      <NavItem href="/trash" label="Corbeille"><Trash2 className="w-5 h-5" /></NavItem>
      <NavItem href="/settings" label="Paramètres"><Settings className="w-5 h-5" /></NavItem>
    </nav>
  );
}
